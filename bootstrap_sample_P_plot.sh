#!/bin/bash
#对每一个采样间隔（例如每0.1ns、0.5ns等），使用 bootstrap 对该采样间隔下的 PCA 投影数据进行重复采样
...
plot_PCA_bootstrap_dir="${plot_PCA_dir}"/P_bootstrap
...
create_dir_if_not_exists "$plot_PCA_bootstrap_dir"

#set environment
export PATH=/home/duanqi/miniconda3/envs/py36_env/bin:$PATH
...
python /home/duanqi/dq/scripts/bootstrap_sample_P_plot.py "${pdb_file}" "${af_file}" "${min_interval}" "${max_interval}" "${num_intervals}" "${plot_PCA_bootstrap_dir}" "${save_time}"
