...
def parse_arguments():
    ...
    parser.add_argument('pdb_file', type=str,
                        help='Path to PDB RMSF of trajectory file')
    
    parser.add_argument('af_file', type=str,
                        help='Path to AF RMSF of trajectory file')
    
    parser.add_argument('plot_RMSF_dir', type=str,
                        help='Directory to save RMSF figure')
...
def main():
    ...
    print(f"  Path to PDB RMSF of trajectory file: {args.pdb_file}")
    print(f"  Path to AF RMSF of trajectory file: {args.af_file}")
    print(f"  Directory to save RMSF figure: {args.plot_RMSF_dir}")
    print(f"  MD length used to name figure: {args.save_time}")

    # Get data
    try:
        print("Reading PDB file...")
        pdb_res_number, pdb_RMSF = read_xvg(args.pdb_file)
        print("Reading AF file...")
        af_res_number, af_RMSF = read_xvg(args.af_file)
    except Exception as e:
        print(f"Error reading XVG files: {str(e)}")
        sys.exit(1)

    #find max and min in all x(pc1) and y(pc2)
    all_x = np.concatenate([pdb_res_number, af_res_number])
    all_y = np.concatenate([pdb_RMSF, af_RMSF])
    ...
    plt.plot(pdb_res_number, pdb_RMSF, '-', color='grey', linewidth=0.4, label='PDB trajectory RMSF', alpha=0.5)
    plt.plot(af_res_number, af_RMSF, '-', color='blue', linewidth=0.4, label='AF trajectory RMSF', alpha=0.5)

    #Add title and label
    plt.title('RMSF of PDB trajectory and AF trajectory', fontsize=16)
    ...
    #Setting plot path
    plot_path = os.path.join(args.plot_RMSF_dir, f'RMSF_{args.save_time}.png')
    plt.savefig(plot_path, dpi=300)
    print(f"Saved RMSF plot to {plot_path}")
