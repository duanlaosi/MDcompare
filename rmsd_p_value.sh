...
plot_RMSD_p_dir="${plot_dir}"/RMSD/P_value
...
create_dir_if_not_exists "$plot_RMSD_p_dir"

#define xvg files
pdb_file=""${pdb_rmsd_dir}"/traj_internal_rmsd.xvg"
af_file=""${af_rmsd_dir}"/traj_fitpdb_rmsd.xvg"
...
python /home/duanqi/dq/scripts/rmsd_p_value.py "${pdb_file}" "${af_file}" "${min_interval}" "${max_interval}" "${num_intervals}" "${plot_RMSD_p_dir}" "${save_time}"
